...
async function main() {
	...
	async function loopCategories(categories) {
		for (const category of categories) {
			// if(category.children.length) {
			// 	loopCategories(category.children)
			// }
			
			// Remove
			if(!category.mediasCount) {
				console.log(`Categoria ${category.name} está vazia (${category.mediasCount}) e será ocultada!`);
				try {
					req = await axios.request({
